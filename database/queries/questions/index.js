module.exports = {
  ...
  findQuestion:
    "SELECT q.id, q.uid, q.date ,q.answered, q.views, q.title, q.uniquetitle, q.body, q.description, q.tags,cardinality(q.upvoters)-cardinality(q.downvoters) AS votes, $2=ANY(q.upvoters) AS upvoted, $2=ANY(q.downvoters) AS downvoted, u.username FROM questions q JOIN users u ON q.uid=u.id WHERE q.id=$1",
