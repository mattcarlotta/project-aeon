module.exports = {
  ...
  findQuestion: `UPDATE questions SET views=views+1 WHERE id=$1; 
    SELECT q.*, cardinality(q.upvoters)-cardinality(q.downvoters) AS votes, $2=ANY(q.upvoters) AS upvoted, $2=ANY(q.downvoters) AS downvoted, u.username, u.reputation as userrep FROM questions q JOIN users u ON q.userid=u.id WHERE q.id=$1
    `,
  findUpdatedQuestion:
    "SELECT q.id, q.userid, q.date, q.body, q.answered, q.views, cardinality(q.upvoters)-cardinality(q.downvoters) AS votes, $2=ANY(q.upvoters) AS upvoted, $2=ANY(q.downvoters) AS downvoted, q.title, q.uniquetitle, q.tags, u.username, u.reputation as userrep FROM questions q JOIN users u ON q.userid=u.id WHERE q.id=$1",
  findAllQuestionsByTagLimitAndOffset:
    "SELECT q.id, q.userid, q.date, q.body, q.answered, q.views, cardinality(q.upvoters)-cardinality(q.downvoters) AS votes, $4=ANY(q.upvoters) AS upvoted, $4=ANY(q.downvoters) AS downvoted, q.title, q.uniquetitle, q.tags, u.username, u.reputation AS userrep FROM questions q JOIN users u ON q.userid = u.id WHERE q.tags @> $1 ORDER BY q.date DESC LIMIT $2 OFFSET $3",
  upvoteQuestion:
